...
@section('breadcrumb', Str::upper('Daftar Pelanggan'))
@section('content')
    <x-bread-crumbs :items="[['text' => 'Daftar Pelanggan', 'url' => '/pelanggan/list']]" />
    <div class="row">
        <div class="col-12 col-lg-12">
            ...
            @php
                $thead = ['', 'No', 'Tanggal', 'Nama Pelanggan', 'Jenis Kelamin', 'No.Handphone', 'email', 'Alamat'];
            @endphp
            <x-card class="shadow-sm rounded-0" bodyClass="text-bg-light shadow-sm border border-light p-0"
                ...
                <div class="d-flex flex-wrap justify-content-lg-between align-items-center flex-column flex-lg-row p-3">
                    @include('Pelanggan.partials.informasi', ['pelanggan' => $pelanggan])
                    @include('Pelanggan.partials.pagination', ['pelanggan' => $pelanggan])
                </div>
            </x-card>
        </div>
