...
class StokRepository
{
    public function getListWithCache(array $filters)
    {
        ...
        $cacheKey = 'stok_barang_' . auth()->id() . '_' . md5("limit={$limit}&keyword={$keyword}&sort={$sortOrder}&page={$page}");

        return Cache::remember($cacheKey, now()->addMinutes(5), function () use ($limit, $keyword, $sortOrder) {
            return StokBarangModel::join('tb_barang', 'tb_stok.id_barang', '=', 'tb_barang.id_barang')
                ->select('tb_stok.*', 'tb_barang.nama_barang')
                ->when($keyword, function ($q) use ($keyword) {
                    $q->where(function ($subQuery) use ($keyword) {
                        $subQuery->where('tb_barang.nama_barang', 'like', '%' . $keyword . '%')
                            ->orWhere('kode_stok', 'like', '%' . $keyword . '%')
                            ->orWhere('no_warehouse', 'like', '%' . $keyword . '%');
                    });
